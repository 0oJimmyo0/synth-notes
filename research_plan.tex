...
\paragraph{Utility endpoint and immediate gate.} A new outcome-unopened patient-grouped split was frozen from original train/dev patients (seed 20260901): 206,111 train notes, 44,382 development notes, and 45,249 unopened-test notes.  Train/dev-only feasibility supports `30_day_same_system_non_elective_return`: the first source-system admission more than 24 hours and at most 30 days after discharge, excluding `ELECTIVE` and `SURGICAL SAME DAY ADMISSION` returns.  Prevalence is 18.1\% in train and 17.7\% in development.  This is an observed same-system non-elective-return endpoint, not all-cause readmission or a validated unplanned-rehospitalization phenotype.  The one concrete next step is a train-only V4 augmentation-eligibility pilot: freeze source selection and pilot size, perform source-ledger and contract review only in utility-train patients, and estimate source-complete and contract-ready yield.  No augmentation rendering, utility-model training, hyperparameter selection, or utility-test outcome access occurs until that pilot freezes the usable augmentation cohort and analysis design.
