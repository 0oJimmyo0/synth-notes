...
The first global-axis pilots, local additive pilots, and the recent projected-boundary pilot were all scientifically useful, but none yet justify scale-up. The main lesson is now clear:
\begin{quote}
...
\end{quote}
Therefore, the redesign must be adapted to the actual ELM interface: a \textbf{single 1024-d source embedding injected through a small adapter}. The next method should preserve local manifold structure and make the steering \textbf{source-conditioned}, instead of relying on one global direction or one uniform projection rule.
...
\begin{enumerate}[nosep]
  ...
  \item freeze the current linear additive family as a negative/partial finding on hard anchors,
  \item redesign the operator around explicit \textbf{target-vs-competitor boundary crossing},
  \item require a hard-anchor pre-decode win before any new large decode run,
  \item only then decide whether the method is ready to scale to other sparse basins found in the full real-manifold map.
\end{enumerate}
...
The first global axis-bank pilots were scientifically useful, but not strong enough to scale. Current local additive steering for cluster 29 should now be treated as a \textbf{negative/partial finding on hard anchors}: it improves target-vs-competitor margin, but still cannot produce a nonzero exact target-cluster win rate on the strongest held-out anchors. The projected-boundary pilot is also a \textbf{negative finding}: despite clean generation and PASS audit, it produced almost no exact cluster-29 occupancy after decoding. Therefore, the immediate next step is \textbf{competitor-aware redesign}, not more broad generation from the current additive/projection families.
...
Phase 2b local-direction redesign & \textbf{Active} & Test whether local target-specific steering beats matched vanilla and random shift on the same anchors \\
LLM editor / external reference / NER & Phase 3 & Start only after a convincing CAV enrichment win \\
\bottomrule
\end{tabularx}
...
\begin{itemize}[nosep]
  \item at least one steering condition improves target-region coverage,
  \item that condition also preserves quality and faithfulness,
  \item and it beats both matched vanilla and norm-matched random shift.
\end{itemize}

\textbf{Stage 3 near-term redesign checklist.}
\begin{itemize}[nosep]
  \item Freeze the current projected-boundary family as a negative result.
  \item Keep the best local additive cluster-29 pilot only as the current internal baseline, not as the final method.
  \item Redesign the steering operator around \textbf{source-conditioned local transport} and \textbf{on-manifold correction}.
  \item Run pre-decode geometry audit before any new scaled generation.
  \item Only decode new pilots after the embedding-level operator shows target-entry gain on the same anchors.
\end{itemize}
