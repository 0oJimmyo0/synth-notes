...
%=============================================================================
\section*{Current frozen primary result (August 2026)}
%=============================================================================

\textbf{Status.} The primary held-out note-quality evaluation is complete and
frozen. This update supersedes the historical steering and pilot-stage roadmap
below as the current decision document. It is not a manuscript draft and no
submission decision should be made until the remaining bounded experiments are
complete and reviewed by the mentor.

\textbf{Frozen cohort flow.}
\begin{center}
\begin{tabular}{lr}
\toprule
Stage & Cases \\
\midrule
Frozen patient-unique test screen & 400 \\
Source-ledger ready & 239 (59.8\%) \\
Manual contract eligible & 208 (87.0\% of source-ready) \\
Compiled hybrid ready and generated & 205 \\
Support- and constraint-selected outputs & 193 (94.1\% of generated) \\
Strict factual-usability passes & 175 (90.7\% of selected; 43.8\% of screen) \\
\bottomrule
\end{tabular}
\end{center}

\textbf{Frozen clinical endpoint.} Blinded source-grounded review of 193
selected outputs found 175 strict passes, 10 unsupported-major-claim flags,
and zero critical omissions. Pass rates were 90/98 (91.8\%) in the
patient-disjoint stratum and 85/95 (89.5\%) in the patient-overlap stratum.
Required diagnosis, active discharge-medication, disposition, and instruction
obligations were retained; residual factual failures localized to free-form
hospital-course prose.

\textbf{Failure mechanism.} The 18 failed outputs were categorized as
redaction/unknown reconstruction (8), temporal or medication-state
contradiction (3), other narrative hallucination (3), unsupported medication
history/action (2), unsupported outcome/procedure assertion (1), and template
artifact (1). Thus deterministic contract rendering controls high-risk
transition omissions, while unconstrained narrative generation remains the
dominant error surface.

\textbf{Locked claim boundaries.} Report conditional factual usability
(175/193) separately from end-to-end pipeline yield (175/400). Do not claim
clinical safety, autonomous clinical use, privacy preservation, unrestricted
public data release, target-region enrichment, or downstream utility. The
frozen test cohort, selected outputs, review labels, renderer, and selection
rule must not be modified, rerun, or used for further optimization.

\textbf{Observed-return utility no-go.} The note-linked hospital-observed
return feasibility audit found adequate linkage overall but inadequate events
for the patient-disjoint stable-sparse test stratum: 27/686 at 30 days and
37/686 at 90 days, compared with 224/6,105 and 333/6,105 in stable dense.
Patient-overlap events must not be used to compensate for this limitation.
Thus observed 30- or 90-day return is not the primary sparse-versus-dense
augmentation endpoint for this study.

\textbf{Renderer decision branch.} Deterministic verified-course rendering
passed 27/27 in the initial development ablation, 17/17 versus 14/17 for the
hybrid course in an arm-blinded paired development comparison, and 11/11 in a
separate development replication. After preserving course facts verbatim and
routing active transitions into deterministic sections, v4 passed 15/15 in a
new development validation. These results support deterministic rendering as
the provisional source-faithful method, but they are not final held-out
performance estimates.

\textbf{Negative configuration evidence.} On held-out notes, deterministic
v3 passed 73/80 with zero unsupported major claims but four critical omissions.
The v4 renderer paired with the older, unrouted contract passed 60/80 with 19
critical omissions and one medication-state contradiction. A targeted audit
showed that all 20 failures had identical old and purportedly routed contract
payloads. Thus the source-faithful method must be evaluated as a locked pair:
an atomic transition contract plus deterministic verbatim-course rendering.
Those already reviewed held-out cases are diagnostic only and cannot be reused
for confirmation.

\textbf{Results-first next gates.}
\begin{enumerate}[nosep]
  \item Compile and structurally audit the targeted contract remediation, then
  lock the contract-routing protocol and deterministic v4 renderer together.
  \item Select a new patient-unique held-out cohort whose notes and outputs have
  not been inspected during renderer development. Predeclare its strict-review
  sample, strata, endpoint, and no-rerun rule.
  \item Obtain blinded independent clinician/pharmacist concordance on all
  failures plus a stratified random subset of passes. Same-environment model
  review is not independent clinical validation.
  \item Evaluate whether the locked pair preserves target-region landing. If it
  does not, remove sparse-manifold enrichment from the primary claim rather
  than substituting clinical fidelity for geometry.
  \item Identify a feasible non-circular downstream outcome before any
  augmentation experiment; observed 30- and 90-day return remain no-go
  endpoints. Complete bounded copying/privacy analysis and plan any release as
  a controlled MIMIC/PhysioNet-derived resource rather than an open dataset.
\end{enumerate}

\textbf{Decision rule.} No manuscript drafting or submission request occurs
until these gates are complete and the mentor reviews the resulting evidence.
...
For Phase 3a, the independent local judge is planned as the gated, locally provisioned \texttt{google/medgemma-27b-text-it} model. Its Hugging Face access terms must be accepted and its read token may be used only to provision an immutable model-only snapshot on scratch; all MIMIC-derived inputs and outputs remain offline on approved project storage. Before any development ledger is read, the model must pass offline BF16 loading and fabricated medication-reconciliation JSON/evidence smoke tests. This model choice does not change the calibration boundary: cluster 25 remains development-only, cluster 11 stays locked, and a third prospective region is required before a judge can become a candidate-triage layer.

The initial full-schema MedGemma calibration attempt established an operational constraint rather than a clinical result: the model remained active on two A40 GPUs but had not completed a single repeat after more than three hours because the prompt required unbounded medication tables and fact-to-note alignments. Phase 3a therefore uses a bounded compact contract: only verified discharge-medication and medication-action instruction facts are provided, and the model returns at most twelve evidence-cited discrepancy findings plus a reconciliation pass/reject decision. The output cap is 1536 tokens and every cap hit, malformed schema, inconsistent decision, or repeat disagreement routes to human review. The configuration must first pass fabricated end-to-end tests and a small label-free cluster-25 runtime preflight before the 60-case development calibration; this is a computational and format gate, not a change to the locked evaluation design.

The compact cluster-25 development calibration is now technically complete: all 60 development tasks completed across three low-temperature repeats on two A40 GPUs in 39 minutes 36 seconds. This initial compact trial used a 768-token cap; of 180 outputs, 177 were schema-valid and 178 were EOS-terminated. Two cap-hit outputs and repeat disagreement in eight of 60 cases are review routes by design. The conservative route detected both known severe development medication errors but also routed fifteen severe-error-negative cases. These are not assumed to be model false positives because the original labels did not adjudicate every model-identified omission for materiality. A label-blind adjudication of all routed cases is therefore required before prompt refinement, threshold selection, or the locked cluster-11 evaluation. This preserves the role of Phase 3a as a calibration study rather than an automatic clinical gate.

That adjudication is complete. All 18 V2 routes were appropriate for human review, but only five model findings were supported and four were material. The adjudication identified two additional real medication omissions that the original focused labels had not classified, while documenting twelve unsupported overcalls and one uncertain duplicated/truncated medication list. Therefore the valid current role is emph{human-review routing}, not autonomous rejection. The V3 development prompt explicitly distinguishes active discharge medications from inpatient treatments, conditional instructions, and action-only evidence. It separates `final_reject` for a supported material discrepancy from `requires_human_review` for uncertainty or a possible discrepancy. V3 will be calibrated only on the existing cluster-25 development set against the derived four-positive material-discrepancy reference, with a 1536-token cap; cluster 11 remains locked until this configuration and its decision policy are frozen.

\noindent\textbf{Phase 3a decision: critical-fact contract and hybrid note assembly.}
The local MedGemma v3.2 experiments are frozen as negative feasibility evidence for autonomous clinical routing. The model was operationally stable, but the independent cluster-36 anchor-disjoint evaluation missed a severe, ledger-supported vancomycin discharge-regimen omission and overcalled a clinically passing note. It must not be tuned further on that evaluation cohort, used as a hard gate, or used to trigger an editor. It remains an optional exploratory review assistant only.

The subsequent contract-bound MedGemma alignment redesign passed all six fabricated release cases, but its locked contract-matched 12-note calibration does not support a safety claim. Of 92 blinded human obligation labels, only 63 had complete schema-valid three-repeat model coverage. The model agreed on 62 of those 63 present-supported-heavy obligations, but missed the sole human unsupported negative-discharge constraint: a generic instruction to resume preadmission medications beyond the explicitly enumerated regimen. Thus observed non-present sensitivity was 0/1; the apparently high covered agreement is not evidence of clinical safety. Generic catch-all active-discharge medication claims must therefore be routed by deterministic code for human review, and an expanded fabricated adversarial benchmark must pass before another clinical LLM evaluation. The local LLM remains a citation-oriented exploratory review aid, never an automatic gate or editor.

The active Phase 3a question is therefore: can mandatory transition-of-care facts be guaranteed in the final note while retaining a useful, source-grounded hospital-course narrative and final-output region selection? Each manually verified ledger will be converted to a versioned \texttt{transition\_note\_contract\_v1}. Every fact is marked \texttt{required}, \texttt{optional}, \texttt{historical\_context\_only}, or \texttt{explicit\_none}; required medication facts additionally specify identity, action, dose, route, frequency/timing, duration, and their required discharge-medication section. A historical inpatient therapy cannot satisfy an active discharge-medication requirement.

Before further GPU generation, a deterministic contract audit will be backtested on existing reviewed free-form candidates. It will record fact-level section coverage, missing components, unsupported additions, and action/dose/route/frequency/duration contradictions. The initial safety criterion is detection of every known severe medication omission, including the vancomycin dialysis regimen; fact-level recall, note-level severe-error sensitivity, false-rejection rate, and human-review referral rate will be reported transparently. Imperfect parsing of historical free-form notes does not by itself block the hybrid pilot, provided the known severe failures are either detected or made impossible through deterministic rendering.

The hybrid pilot will deterministically render principal diagnosis, active discharge medications, disposition, verified instructions, and verified follow-up when present. ELM will generate only a constrained hospital-course narrative from verified course facts; it will not generate a discharge medication list, disposition, or follow-up assertion. Each assembled final note must pass the full fact contract, structural and privacy gates, then be BGE re-embedded. Geometry ranking occurs only among contract-passing candidates. The pilot compares the frozen fact-only free-form arm with the hybrid arm on 8--12 new, source-complete, anchor-disjoint cases. Scaling remains blocked until the hybrid arm shows zero unsupported active discharge medications or severe critical omissions in blinded review, adequate final-output target-region yield versus a matched control, and reproducible blinded quality in more than one independent cohort.

\noindent\textbf{Revised mechanism and claim boundary.}
The project no longer claims that an inference-time edited embedding is reliably decoded into a faithful sparse-region note. Instead, embedding-space analysis identifies under-covered real-note regions and selects held-out anchors; a verified compact fact contract controls high-risk transition fields; constrained local synthesis supplies only the hospital-course narrative; and BGE re-embedding audits whether the assembled final output enriches the intended real manifold. This preserves the cohort-enrichment objective while separating anchor selection, deterministic factual control, narrative generation, and output-space evaluation.
...
\subsection{Frozen hybrid regression guard}

The hybrid contract pipeline is now the active generation method: deterministic rendering supplies diagnosis, active discharge medications, disposition, and instructions, while ELM generates only hospital-course prose from reviewed course facts.  In the first 25-case Region-25 hybrid cohort, a contract-reference correction showed that one apparent medication omission concerned facts explicitly marked historical-only rather than active discharge obligations.  The remaining genuine failure was an unsupported ``post-operative day 0'' statement in free-form course prose.  A source-conditioned guard now rejects numbered postoperative-day claims unless the same day is present in the reviewed course facts.  Its one-case, four-candidate regression achieved target-cluster landing and a blinded 5/5 factual/clinical pass.  This is a regression result, not a scale-up result.  The next test must freeze this version and evaluate it on a new independent, anchor-disjoint cohort with final-output geometry selection and blinded contract-aware review.

\subsection{Canonical transition-note representation pivot}

The original real-note geometry is retained only as a frozen sensitivity endpoint.  A paired 28-case audit found that the locally cached BGE tokenizer has a 512-token limit: all raw real notes were truncated (median 2,068 tokens), whereas only 2/28 canonical scaffold and 2/28 hybrid transition notes were truncated.  Raw notes began with administrative/admission content, while hybrid notes began with discharge diagnosis.  Thus the original target is effectively a first-512-token raw-note representation and is not a fair primary comparison for compact transition-note outputs.  The raw-space Region-44 ablation is frozen as a diagnostic result (25.0\% transition-only, 39.3\% plus verified course, 42.9\% scaffold/full-hybrid candidate landing, and 46.4\% best-of-four anchor yield), not as a full-discharge-note enrichment claim.

The next no-generation experiment defines a common canonical transition-note representation for real and hybrid notes: discharge diagnosis, brief hospital course, active discharge medications, disposition, instructions, and explicit follow-up only.  Each section will be chunked within the frozen BGE length limit, embedded separately, averaged within section, then aggregated with equal weights across required sections and L2-normalized.  Geometry must be refit on real canonical train/dev notes and assigned to held-out real test notes before frozen hybrid outputs are evaluated.  The pivot proceeds only if canonical regions are stable and clinically interpretable, same-anchor hybrid alignment improves beyond random selection, and results are not driven by section order or one section.  Otherwise the next representation is a fact-contract space, not further prompt tuning or larger candidate counts.

Before making the canonical representation primary, a frozen three-way no-generation comparison will separate truncation from information-scope mismatch: (i) the legacy prefix embedding, (ii) a chunked full-note embedding using 448 content-token chunks with 64-token overlap and token-count-weighted aggregation, and (iii) the canonical section-balanced embedding.  On the fixed 28 held-out anchors and all frozen hybrid candidates, report same-anchor cosine, real-anchor retrieval rank, neighbor stability, and patient-disjoint sensitivity.  These outputs are diagnostic only and cannot define a new target region after synthetic outputs are observed.  Any subsequent enrichment claim must fit geometry on real canonical or chunked train/dev notes, then assign held-out real test anchors and synthetic test outputs in that same frozen space.

The frozen 28-anchor audit confirmed that truncation is material: chunked full-note embedding improved mean same-anchor cosine from 0.779 to 0.853, mean real-anchor retrieval rank from 4.35 to 1.30, and top-1 retrieval from 53.6\% to 87.5\%.  Low legacy--chunked neighbor overlap confirms that a new representation requires a new geometry fit.  The contract-scaffold section-balanced comparison reached near-perfect same-anchor retrieval, but it is an upper bound rather than an independent result because the scaffold and hybrid notes share reviewed contract content.  Before a canonical manifold is claimed, the same sections must be extracted deterministically from raw real notes without using the synthetic contract, their provenance and coverage audited, and the comparison repeated.  Chunked full-note BGE is therefore the leading least-disruptive representation for the next train/dev geometry rebuild, while canonical section-balanced BGE remains a prespecified competing primary representation pending this independent extraction test.

That independent raw-heading rerun is complete.  Twenty-six of the 28 frozen anchors contained all five required source headings; across their 104 frozen hybrid candidates, raw-heading canonical section-balanced BGE achieved mean same-anchor cosine 0.941, mean retrieval rank 1.00, and 100\% top-1 real-anchor retrieval.  Chunked full-note BGE improved over legacy prefix BGE but remained lower (mean cosine 0.854 versus 0.777 legacy; top-1 86.5\% versus 53.8\% legacy).  This supports a section-matched transition-note representation as the candidate primary manifold and retains legacy-prefix and chunked-full-note BGE as frozen sensitivities.  It remains an alignment diagnostic, not an enrichment result: the next gate is a prespecified canonical extraction over real train/dev notes, geometry fitting without synthetic outputs, held-out real-test assignment, and only then frozen hybrid-output evaluation.

\subsection{Canonical-manifold confirmatory program}

The primary analysis object is the empirical manifold of \emph{source-complete discharge-transition notes}: discharge diagnosis, brief hospital course, active discharge medications, disposition, and actionable instructions.  Follow-up is included only when directly supported.  The project does not infer missing sections, reconstruct source text from a reviewed contract, or claim complete raw discharge-summary synthesis.  Source completeness is therefore an explicit eligibility criterion and its rate, exclusion reasons, and subgroup variation must be reported across the cohort.

The frozen specification \texttt{canonical\_transition\_bge\_v1.json} defines accepted headings, section order, duplicate-heading handling, explicit-none handling, 448-token chunks with 64-token overlap, token-count-weighted within-section pooling, equal weighting across the five required section vectors, and final L2 normalization.  Legacy-prefix and chunked-full-note BGE are sensitivity representations only.  The specification is hashed into all extraction, embedding, and geometry artifacts and may not be tuned using held-out synthetic outputs.

A development-only heading-label audit found that v1 excluded some notes because of semantically clear but unrecognized headings, especially \emph{Primary Diagnosis} and \emph{Followup Instructions}.  A conservative \texttt{canonical\_transition\_bge\_v2.json} therefore adds only explicit primary-diagnosis, specialty-course, and follow-up-instruction aliases; it expressly excludes secondary diagnoses, emergency-department course, reason-for-hospitalization, admission-medication, and home-medication headings.  v2 must reproduce its development gain and pass one frozen Region-44 test-anchor extraction regression before train extraction.  No test heading may motivate a subsequent alias change.

The v2 development gate completed on all 32,847 dev source rows with no missing source text.  It retained 21,408 notes (65.2\%), increasing the v1 source-complete population by 4,292 notes.  Residual missingness is chiefly diagnosis or hospital-course headings, and duplicate-heading excess is rare and governed by the frozen first-match policy.  The only remaining extraction gate is the single frozen Region-44 test-anchor regression; a clean result freezes v2 and permits train extraction without any further alias changes.

The held-out Region-44 extraction regression passed: all 30 requested test anchors loaded and all 30 were source-complete under v2, with no duplicate-heading excess or missing source text.  This validates, but does not modify, the development-derived specification.  Canonical v2 is now frozen for train/dev extraction; the next analysis reports source-completeness and representativeness before BGE embedding or geometry fitting.

The train/dev eligibility audit labeled all 295,742 real notes under frozen v2 and retained 65.60\% overall (65.65\% train; 65.17\% dev), with no missing source text.  This defines the source-complete primary population, not the full discharge-summary cohort.  Eligibility varies materially by service (medicine 58.1\%, psychiatry 42.9\%, surgery 80.6\%, orthopedics 85.4\%, gynecology 91.9\%), age, admission type, insurance, and length of stay.  These reachability differences require subgroup-stratified reporting and preclude a full-cohort claim, but do not block canonical embedding of the eligible real population.  The next gate is a v2 held-out anchor alignment regression, followed by train-only geometry fitting with dev-only stability selection.

Canonical v2 embedding is complete for 172,586 source-complete train notes and 21,408 source-complete dev notes, with deterministic two-worker merge coverage verified.  The frozen 28-anchor alignment diagnostic supports the section-balanced representation (mean same-anchor cosine 0.938 and 100\% top-1 retrieval over 112 hybrid candidates), but this is an alignment result only.  A real-train-only MiniBatch K-means grid with dev-only seed stability ($K=25,50,75,100$) did not support discrete canonical target regions: mean dev ARI was 0.477--0.581 and the nominal $K=100$ selection contained a two-note dev cluster.  This is a negative partition-stability diagnostic; no test assignment, target selection, or generation may use those cluster labels.

The next geometry gate is therefore continuous and group-aware.  For five prespecified subject-hash train splits, estimate each dev note's \emph{subject-excluded local support} as mean top-$k$ cosine similarity to real-train neighbors, for $k\in\{10,25,50,100\}$.  A source patient must occur in only one reference half, and train rows sharing the dev subject must be excluded from that dev query's reference neighborhood.  Report each $k$ separately: median/minimum support-rank agreement, median/minimum sparse-decile Jaccard, per-anchor sparse frequency, and patient-disjoint-dev sensitivity.  ``Local support'' is deliberately not called probability density.

Advancement requires project-specific, prespecified operational criteria: median dev rank Spearman at least 0.90, minimum repeated-split Spearman at least 0.85, median sparse-decile Jaccard at least 0.60, stability at two adjacent $k$ values, and anchor sparse frequency at least 80\% across splits at the selected and adjacent $k$.  These are study decision criteria, not universal constants.  A valid continuous under-coverage target must then combine reproducible real local support, unexpectedly weak canonical vanilla support conditional on real support, source eligibility, and clinical interpretability; sparse real support alone is not an enrichment target.  The support/vanilla-deficit rule must be frozen on dev before one held-out test application.  If these gates fail, geometry remains a secondary source--output alignment endpoint and the primary contribution becomes fact-contract-controlled generation rather than a forced cohort-level geometric-enrichment claim.

The confirmatory sequence is: (i) streaming raw-heading extraction and eligibility audit over real train/dev notes; (ii) canonical embeddings plus deterministic-scaffold, hospital-course-only, and leave-one-section-out ablations; (iii) geometry fitting on real train, with dev-only stability and hyperparameter selection; (iv) frozen assignment of real test notes and existing hybrid outputs; and (v), only after those gates, a multi-region test using the frozen contract-first hybrid generator.  The primary endpoint is anchor-level joint success: an eligible held-out anchor has a selected synthetic output that enters a prespecified under-covered canonical region and passes contract, privacy, and blinded clinical-validity gates.  Selection is compared with a random one-of-$K$ candidate from the identical candidate pool.  A manuscript-scale study additionally requires multiple prespecified regions, patient-disjoint reporting, blinded review with agreement assessment, privacy evaluation, and downstream utility on held-out real data; it must not represent MedGemma as an autonomous gate or editor.

\paragraph{V4 renderer confirmation (frozen).} A fresh balanced held-out screen of 80 unique-subject notes was reviewed under the V4 source and contract protocol.  Forty-seven cases passed source review, 45 passed required canonical-section validation (two lacked actionable instructions), and 34 remained after contract-level exclusion of 11 materially incomplete transitions.  The locked deterministic pair---an atomic reviewed transition contract plus the V4 renderer with verbatim reviewed course facts---passed blinded strict review in 34/34 contract-eligible cases, with all 307 required facts present in their designated sections, zero unsupported major claims, and zero critical omissions.  Mean consistency and factual-faithfulness scores were both 4/5 because source-preserving placeholders, terse fragments, and repetition reduce readability without changing the represented action.  This is evidence for source-faithful deterministic rendering among prescreened eligible cases; it is not evidence of unrestricted 34/80 performance, independent external-clinician validation, target-region enrichment, privacy safety, public release suitability, or downstream clinical utility.

\paragraph{V4 bounded privacy characterization (frozen).} Exact lexical screening against all real train notes, excluding every same-subject train note per output, found no unrelated exact span of at least five tokens and no limited-pattern PHI-like hit.  Source retention was substantial by design (mean source-token coverage 60.0\%; mean hospital-course coverage 92.4\%).  Canonical BGE screening gave mean output-to-source cosine 0.959 and mean nearest patient-excluded-train cosine 0.915.  One negative and two near-zero source-minus-nearest margins underwent restricted review; all were common clinical semantics rather than patient-specific similarity or likely unrelated-training memorization.  Thus the bounded screens found no evidence of additional unrelated-training memorization.  They do not establish privacy safety: the verbatim-source design makes V4 a restricted source-grounded transformation and precludes a public-release or privacy-preserving claim.

\paragraph{Utility endpoint and immediate gate.} A new outcome-unopened patient-grouped split was frozen from original train/dev patients (seed 20260901): 206,111 train notes, 44,382 development notes, and 45,249 unopened-test notes.  Train/dev-only feasibility supports `30_day_same_system_non_elective_return`: the first source-system admission more than 24 hours and at most 30 days after discharge, excluding `ELECTIVE` and `SURGICAL SAME DAY ADMISSION` returns.  Prevalence is 18.1\% in train and 17.7\% in development.  This is an observed same-system non-elective-return endpoint, not all-cause readmission or a validated unplanned-rehospitalization phenotype.  The immediate gate is an outcome-blinded, train-only V4 augmentation-eligibility scalability pilot of 400 unique patients, replacing the earlier 80-patient exploratory size.  Before sampling, the available metadata must be audited for a reliable service label; service must not be inferred from note text or outcomes.  The frozen pilot must report source-complete, ledger-ready, and contract-ready yield; exclusion reasons; correction and atomic-obligation burden; and reviewer time overall and by available service, note-length, and contract-complexity strata.  No augmentation rendering, utility-model training, hyperparameter selection, or utility-test outcome access occurs until this pilot establishes a usable, operationally feasible cohort and freezes the augmentation analysis design.

\paragraph{Manuscript decision gate.} The V4 34-case confirmation is a component-validation result, not a manuscript-ready synthetic-data claim.  Manuscript drafting remains deferred until the 400-patient scalability pilot is complete; an automated ledger-to-contract pipeline has been evaluated against held-out manually reviewed gold contracts; a predeclared 2,000--5,000-case controlled augmentation learning curve has compared real-only, duplicated-real, canonical-extractive, deterministic-scaffold, and V4 arms; and the locked utility test has been evaluated only after development decisions freeze.  The final evidence package must also include scaled copying/privacy characterization, robustness by support/service/complexity strata, and independent clinical review of a substantial random output sample (target approximately 150 outputs, ideally with two reviewers or clinician/pharmacist adjudication).  Internal model reviews are not described as independent clinical validation.

\paragraph{400-patient scalability-pilot result.} The frozen outcome-blinded utility-train pilot selected 400 unique patients, balanced at 80 per source-note-length quintile.  The validated funnel retained 126 source-ledger-ready cases (31.5\% of sampled), of which 113 were contract-ready (89.7\% of source-ready; 28.25\% of sampled).  Source readiness was 38.75\%, 36.25\%, 38.75\%, 23.75\%, and 20.0\% from the shortest to longest quintile, respectively; conditional contract conversion was 87.5--94.7\% in every quintile.  These descriptive results identify source completeness and review burden, especially in longer notes, as the central scalability bottleneck rather than final contract routing.  The 113 cases are retained as restricted manually reviewed gold contracts for the next automation experiment; they are not rendered or used for augmentation until a case-level development/held-out-gold contract-automation evaluation is frozen.

\paragraph{Contract-automation provenance gate.} The 126 source-ready cases were split at the case level into 88 development and 38 held-out manual-gold cases, preserving manually excluded cases in both partitions.  An exported development-only v1.0 automation result lacked a recoverable executable, configuration, literal invocation, or hash trace after working-tree, Git-history, reflog, unreachable-object, shell-history, and job-log recovery.  It is therefore non-reproducible at the executable-artifact level and cannot justify opening held-out gold labels.  The next automation experiment is a newly implemented v1.1, developed only on the 88 development cases.  Its code, configuration, development command, inputs, deterministic rerun, and hashes must be frozen before its one-time 38-case held-out evaluation.  The v1.1 primary endpoint is false acceptance of manually excluded cases; automated coverage and obligation/section/component agreement are secondary endpoints.
