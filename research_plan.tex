...
\begin{enumerate}[nosep]
  ...
  \item run a small \textbf{three-arm source-fact-conditioned feasibility study}: compare (A) raw ELM output, (B) local source-fact-conditioned correction of that output, and (C) local source-fact-only generation from the same verified ledger. This isolates whether the ELM draft is a useful clinical scaffold or harmful narrative context,
  \item require improved diagnosis, procedure, complication, medication-change, and disposition preservation against the current near-zero baseline before considering a constrained editor or a larger candidate pool.
\end{enumerate}

\textbf{Current rescue-pilot preparation.}
The completed manual labels have been ingested without copying source text into derived summaries. A reproducible 45-case held-out pilot cohort was selected across exact-pooled and centroid-only routes with 10 patient-disjoint cases. Manual ledger review transformed 406 provisional facts into a completed restricted ledger of 483 rows: 354 verified, 82 corrected, and 47 rejected, with 77 reviewed source-supported additions. All 45 cases now have verified/corrected coverage for the six required fields, and independent source-span validation is 100\% for the 436 usable facts. The next implementation dependency is an approved \textbf{local} model/interface for the correction and fact-only arms; no source-derived text may leave approved storage.
...
The input-space axis, local-transport, and decoder-adaptation families are now \textbf{negative/partial findings}: they clarify decoder compatibility but do not support inference-time enrichment claims. Closed-loop output selection is a positive feasibility signal for final-output basin landing, but the completed clinical and source-paired reviews fail the source-faithfulness requirement. The next milestone is a three-arm source-fact-conditioned feasibility study, not another steering family, a larger candidate pool, or a downstream-NLP experiment.
