...
## August 9 V4 privacy characterization and utility endpoint freeze

- The frozen V4 34-output set passed a restricted copying characterization. Exact lexical screening against all real train notes, excluding every same-subject train note for each output, found no unrelated exact span of five or more tokens and no limited-regex PHI-like hit. Source retention is substantial by design: mean source-token coverage was 60.0%, with hospital-course coverage 92.4%.
- Canonical BGE semantic screening found mean output-to-own-source cosine 0.959 versus mean nearest patient-excluded train cosine 0.915. One output had a small negative source-minus-nearest margin and two were reviewed as a sensitivity queue; all three were adjudicated `common_clinical_semantics_only`, with no suspicious patient-specific similarity or likely unrelated-training memorization. The bounded decision is no evidence of *additional unrelated-training memorization from the evaluated screens*, not a privacy guarantee.
- V4 remains a restricted source-grounded transformation. Verbatim source retention prevents a privacy-preserving or public-release claim even though the bounded lexical, pattern, and semantic screens were negative for unrelated train copying.
- A fresh outcome-unopened patient-grouped utility split was created from original train/dev patients only (seed 20260901): 206,111 train notes/95,001 subjects, 44,382 dev notes/20,608 subjects, and 45,249 unopened-test notes/20,551 subjects. No utility-test outcome has been accessed.
- Train/dev feasibility supports a frozen candidate endpoint named `30_day_same_system_non_elective_return`: the next source-system admission begins more than 24 hours and no more than 30 days after discharge, excluding `ELECTIVE` and `SURGICAL SAME DAY ADMISSION` next admissions. Its train/dev prevalences are 18.1% and 17.7%, respectively. This is not all-cause readmission or a validated unplanned-rehospitalization phenotype.
- **Concrete next step:** run a train-only V4 augmentation-eligibility pilot. Freeze the source-selection rule and pilot size; perform source-ledger and contract review only for those train patients; measure source-complete and contract-ready yield. Do not render augmentation, train a downstream classifier, tune against utility test data, or access unopened-test outcomes until the pilot fixes the usable augmentation cohort and analysis design.

## August 9 mentor-aligned scale and manuscript gate

- The V4 34-case confirmation establishes source-faithful deterministic rendering among prescreened contract-eligible held-out cases. It does **not** yet establish a scalable synthetic-data system, downstream utility, representation-space enrichment, privacy for release, or independent clinical validation. Manuscript drafting is deferred.
- Replace the previously contemplated 80-patient utility-train exercise with an outcome-blinded **400 unique-patient** scalability pilot. Before sampling, audit whether a reliable service variable exists; do not infer service labels from note text or outcomes. At minimum, freeze and report sampling by available source metadata and note-length/contract-complexity strata.
- The pilot must quantify the full operational funnel: source-complete, ledger-ready, and contract-ready yield; exclusion reasons; correction and atomic-obligation burden; reviewer time; and review time by available complexity/service strata. It must not render notes, train utility models, or access utility-test outcomes.
- A subsequent automation study must compare an automated ledger-to-contract pipeline with manually reviewed gold contracts from a split not used to tune the automation. Report required-fact recall and precision, medication-component correctness, unsafe-transition false acceptance, and manual-review burden.
- Scale to 2,000--5,000 eligible augmentation cases only if the 400-patient yield and labor results show feasibility. The later controlled utility study requires real-only, duplicated-real, canonical-extractive, deterministic-scaffold, and V4 arms, development-only tuning, and one untouched utility test.
- Before any manuscript decision, require the completed scalability and automation experiments, a predeclared augmentation learning curve and held-out real-data utility evaluation, scaled copying/privacy characterization, robustness by support/service/complexity strata, and an independent clinical review sample (target approximately 150 outputs, preferably with two reviewers or clinician/pharmacist adjudication). Internal model reviews are not independent clinical validation.

## August 10 utility-train 400-case scalability pilot frozen

- The outcome-blinded pilot selected 400 unique utility-train patients (seed 20260902), balanced at 80 patients in each frozen source-note-length quintile. The utility manifest has no reliable service label; service was explicitly not inferred from note text or outcomes.
- The validated pipeline funnel is: 400 sampled, 126 source-ledger ready (31.5%), 113 contract-ready (89.7% of source-ready; 28.25% of sampled). The original case-level source-safety count must not be reported as source readiness; the validator's required-section completeness gate defines the official 126-case denominator.
