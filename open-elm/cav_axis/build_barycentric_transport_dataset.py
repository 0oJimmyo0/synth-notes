#!/usr/bin/env python3
"""
Build a source-conditioned barycentric transport HF dataset for ELM generation.

This Phase 2b operator keeps the shifted embedding close to the empirical
real-note manifold by mixing each source anchor with a local convex
combination of target-neighborhood embeddings instead of applying a single
global additive direction.
"""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Build a barycentric/local-transport HF dataset.")
    ...
    parser.add_argument(
        ...
        required=True,
        help="Pandas query selecting source anchors, e.g. 'cluster_target_29 == 0 and boundary == 1'",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        default="",
        help="Optional comma-separated competitor cluster IDs, e.g. 9,45,17. If omitted, use plain barycentric transport.",
    )
    parser.add_argument(
        "--split_manifest_path",
        default=None,
    ...
    parser.add_argument(
        "--alphas",
        required=True,
        help="Comma-separated source-vs-target mixing weights, e.g. 0.1,0.2,0.3",
    )
    parser.add_argument(
        "--k_target_neighbors",
        type=int,
    ...
    parser.add_argument(
        ...
        type=int,
        default=16,
        help="Number of nearest competitor neighbors for the chosen best competitor cluster",
    )
    parser.add_argument(
        "--neighbor_weight_mode",
        default="softmax_cosine",
        choices=["softmax_cosine", "inverse_distance", "uniform"],
        help="How to weight the k target neighbors inside the local barycenter",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--competitor_gammas",
        default="0.0",
        help="Comma-separated repulsion strengths for the chosen best competitor neighborhood, e.g. 0.05,0.10",
    )
    parser.add_argument(
        "--max_source_rows",
        type=int,
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="L2-normalize shifted embeddings after the barycentric move",
    )
    parser.add_argument(
        "--output_stem",
        default="barycentric_transport",
...
def compute_neighbor_weights(
    ...
    if mode == "uniform":
        ...
        inv = 1.0 / np.clip(cosine_distances, 1e-6, None)
...
def barycentric_target(
    target_neighbor_embeddings: np.ndarray,
    cosine_distances: np.ndarray,
    mode: str,
    temperature: float,
) -> tuple[np.ndarray, np.ndarray]:
    weights = compute_neighbor_weights(cosine_distances, mode=mode, temperature=temperature)
    mixed = np.sum(target_neighbor_embeddings * weights[:, None], axis=0, dtype=np.float32)
    mixed = normalize_rows(mixed.reshape(1, -1))[0].astype(np.float32)
    return mixed, weights


def top1_per_cluster(
    source_embeddings: np.ndarray,
    ...
    for start in range(0, source_embeddings.shape[0], batch_size):
        stop = min(start + batch_size, source_embeddings.shape[0])
        sims = source_embeddings[start:stop] @ cluster_embeddings.T
...
def main() -> None:
    ...
    competitor_gammas = parse_float_list(args.competitor_gammas)
    competitor_cluster_ids = [int(x) for x in parse_csv_list(args.competitor_cluster_ids)]
    if not alphas:
        raise ValueError("--alphas must specify at least one steering strength")
    if not competitor_gammas:
        raise ValueError("--competitor_gammas must specify at least one value")
    ...
    competitor_df = merged_df.loc[pd.to_numeric(merged_df.get("cluster_id"), errors="coerce").isin(competitor_cluster_ids)].copy()
    ...
    if competitor_cluster_ids and competitor_df.empty:
    ...
    if competitor_cluster_ids:
        for cid in competitor_cluster_ids:
            cid_df = competitor_df.loc[pd.to_numeric(competitor_df["cluster_id"], errors="coerce") == cid].copy()
            if cid_df.empty:
                continue
            cid_row_ids = cid_df["dataset_row_id"].astype(int).to_numpy()
            cid_embeddings = normalize_rows(
                np.stack([extract_embedding_vector(dataset[int(i)]) for i in cid_row_ids], axis=0)
            )
            competitor_cluster_arrays[cid] = (cid_row_ids, cid_embeddings)
            competitor_cluster_top1[cid] = top1_per_cluster(source_embeddings, cid_embeddings)
        if not competitor_cluster_arrays:
            raise ValueError("No usable competitor cluster pools were built from --competitor_cluster_ids.")
    ...
    if competitor_cluster_ids:
        axis_label = f"barycentric_transport_compaware__{args.target_column}__vs__{'_'.join(str(x) for x in competitor_cluster_ids)}"
    else:
        axis_label = f"barycentric_transport__{args.target_column}"
    ...
    for source_order, (_, source_row) in enumerate(source_df.reset_index(drop=True).iterrows()):
        ...
        target_mix, neighbor_weights = barycentric_target(
            local_target_embs,
            cosine_distances=nbr_dist,
        ...
        neighbor_entropy = float(-np.sum(neighbor_weights * np.log(np.clip(neighbor_weights, 1e-12, None))))

        best_competitor_cluster = None
        best_competitor_dataset_row_id = None
        best_competitor_cosine = None
        competitor_mix = None
        competitor_entropy = None
        competitor_neighbor_mean_cosine = None
        if competitor_cluster_arrays:
            competitor_candidates: list[tuple[int, float]] = []
            for cid, (scores, _) in competitor_cluster_top1.items():
                competitor_candidates.append((cid, float(scores[source_order])))
            competitor_candidates.sort(key=lambda item: item[1], reverse=True)
            best_competitor_cluster = int(competitor_candidates[0][0])
            best_competitor_cosine = float(competitor_candidates[0][1])

            comp_row_ids, comp_embeddings = competitor_cluster_arrays[best_competitor_cluster]
            k_comp = min(int(args.k_competitor_neighbors), len(comp_row_ids))
            comp_sims = comp_embeddings @ source_unit
        ...
        for alpha in alphas:
            if not (0.0 <= alpha <= 1.0):
                raise ValueError(f"Barycentric alpha must be in [0, 1], got {alpha}")
            for competitor_gamma in competitor_gammas:
                if competitor_gamma < 0.0:
                    raise ValueError(f"Competitor gamma must be >= 0, got {competitor_gamma}")

                if competitor_mix is not None:
                    competitor_aware_target = ((1.0 + competitor_gamma) * target_mix) - (competitor_gamma * competitor_mix)
                    competitor_aware_target = normalize_rows(competitor_aware_target.reshape(1, -1))[0].astype(np.float32)
                else:
                    competitor_aware_target = target_mix

                shifted_embedding = ((1.0 - alpha) * source_unit) + (alpha * competitor_aware_target)
                raw_shift_norm = float(np.linalg.norm(shifted_embedding - source_unit))
                ...
                target_mix_cosine = float(np.dot(target_mix, shifted_embedding) / shifted_norm)
                competitor_mix_cosine = (
                    float(np.dot(competitor_mix, shifted_embedding) / shifted_norm) if competitor_mix is not None else None
                )
                target_competitor_margin = (
                    float(target_mix_cosine - competitor_mix_cosine) if competitor_mix_cosine is not None else None
                )
                ...
                shifted_row = {
                    ...
                    "alpha": float(alpha),
                    "competitor_gamma": float(competitor_gamma),
                    ...
                    "transport_mode": "barycentric_transport_competitor_aware" if competitor_mix is not None else "barycentric_transport",
                    ...
                    "neighbor_weight_entropy": neighbor_entropy,
                    "local_target_centroid_cosine": target_mix_cosine,
                    "best_competitor_cluster": best_competitor_cluster,
                    "best_competitor_dataset_row_id": best_competitor_dataset_row_id,
                    "best_competitor_anchor_cosine": best_competitor_cosine,
                    "competitor_neighbor_count": int(min(int(args.k_competitor_neighbors), len(competitor_cluster_arrays.get(best_competitor_cluster, ([], []))[0]))) if best_competitor_cluster is not None else 0,
                    "competitor_neighbor_mean_cosine": competitor_neighbor_mean_cosine,
                    "competitor_weight_entropy": competitor_entropy,
                    "local_competitor_centroid_cosine": competitor_mix_cosine,
                    "target_competitor_margin": target_competitor_margin,
                ...
                dataset_manifest_rows.append(
                    {
                        ...
                        "competitor_gamma": shifted_row["competitor_gamma"],
                        ...
                        "neighbor_weight_entropy": shifted_row["neighbor_weight_entropy"],
                        "best_competitor_cluster": shifted_row["best_competitor_cluster"],
                        "best_competitor_dataset_row_id": shifted_row["best_competitor_dataset_row_id"],
                        "best_competitor_anchor_cosine": shifted_row["best_competitor_anchor_cosine"],
    ...
    summary_payload = {
        ...
        "competitor_gammas": competitor_gammas,
        ...
        "n_competitor_rows": int(len(competitor_df)) if competitor_cluster_ids else 0,
